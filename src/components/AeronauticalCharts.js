...
import { 
  FaMap, FaCheckCircle, FaSearch, FaFilter, FaTh, FaList, 
...
const ViewButton = styled.button`
  padding: 12px 16px;
  background: ${props => props.active ? 'var(--color-accent)' : 'var(--color-bg)'};
  color: ${props => props.active ? 'var(--color-text)' : 'var(--color-text-muted)'};
  border: none;
  cursor: pointer;
...
const AeronauticalCharts = () => {
  ...
  const formatPrice = (prices) => {
    ...
    return prices.map(p => `${p.size.toUpperCase()}: $${p.price_usd} USD`).join(', ');
  };
  
  const formatPriceArray = (prices) => {
    if (!prices || prices.length === 0) return [];
    return prices.map(p => `${p.size.toUpperCase()} size: ${p.price_usd} USD`);
  };
      ...
      <ChartPrices>
        {chart.prices && chart.prices.length > 0 ? (
          chart.prices.map((price, i) => (
            <div key={i}>{price.size.toUpperCase()}: ${price.price_usd} USD</div>
          ))
        ) : (
          <div>N/A</div>
        )}
  ...
  const renderChartCard = (chart) => (
    <ChartCard key={chart.id}>
      ...
      <ChartCardInfo>
        <strong>Name:</strong> {Array.isArray(chart.name) ? chart.name.join(', ') : chart.name}
      </ChartCardInfo>
      {chart.scale && (
        <ChartCardInfo>
  ...
  return (
    <ChartsSection>
        ...
        <FilterSelect value={selectedType} onChange={handleTypeChange}>
          ...
          {chartTypes?.map(type => (
            <option key={type.chart_type} value={type.chart_type}>
              {CHART_TYPE_LABELS[type.chart_type] || type.chart_type} ({type.count})
            </option>
          ))}
        </FilterSelect>
        <ViewToggle>
      ...
      <ChartCounter>
        ...
        {selectedType && ` (filtered by ${CHART_TYPE_LABELS[selectedType] || selectedType})`}
      </ChartCounter>
      ...
      {charts.length === 0 ? (
            ...
            <CategorySection key={type}>
              ...
              {!isCollapsed && (
                viewMode === 'table' ? (
                  ...
                  <ChartsGrid>
                    {typeCharts.map(renderChartCard)}
                  </ChartsGrid>
                )
              )}
            </CategorySection>
        ...
        viewMode === 'table' ? (
          ...
          <ChartsGrid>
            {displayedCharts.map(renderChartCard)}
          </ChartsGrid>
        )
      )}
